...
def print_product(num_1, num_2):
    return num_1 * num_2
    print print_product 

def repeat_string(occur_again, times_3):
    return occur_again * 3
    print str(repeat_string)

def print_sign(some_num):
    if some_num > 0:
        print "Higher than 0"
    elif some_num < 0:
        print "Lower than 0"
...
def is_divisible_by_three(possibly_divisible):
    return possibly_divisible / 3
    if is_divisible_by_three == True:
        return possibly_divisible

def num_spaces(take_this_sentence): 
    return num_spaces.count()
